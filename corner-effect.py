...
class Agent:
    ...
    def update(self, target, agents):
        ...
        # Verifica a proximidade de cantos
        corner_proximity = self.get_corner_proximity()
        if corner_proximity < 50:
            self.max_speed = 1
        else:
            self.max_speed = 3
    ...
    def get_corner_proximity(self):
        corner_distance = min(self.position.x, WIDTH - self.position.x, self.position.y, HEIGHT - self.position.y)
